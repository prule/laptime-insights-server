...
/**
 * SQL `DATE_TRUNC(<unit>, <expr>)` wrapped as an Exposed expression. Supported by H2 and PostgreSQL
 * with matching semantics for `DAY`, `WEEK` (Monday start) and `MONTH`. The result is typed as
 * `Instant` so it can be selected into a `ResultRow` and read back via the same column type the
 * source column uses.
 */
 ...
 * `YYYY-MM` for month).
 *
 * **Must stay in sync with the frontend's `bucketStartKey` in `frontend/src/lib/buckets.ts`** — the
 * dashboard joins aggregate responses onto its bucket grid by string equality, so a casing or
 * padding drift between the two sides silently breaks the join.
 *
 * Uses `ZoneId.systemDefault()` on purpose: Exposed's `InstantColumnType` stores `Instant`s as
 * TZ-less wall-clock values via `TimeZone.currentSystemDefault()` and reads them back the same way,
 * so the only way to recover the truncated wall-clock day is to format against the same zone. In a
 * single-user setup the JVM and browser TZ coincide, so the client sees the day the player
 * perceived the event on.
 */
