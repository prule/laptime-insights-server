 ...
 * row does not exist yet at capture time. Once a lap completes and its Lap row is created, this
 * port correlates the still-unlinked frames for that car via `sessionUid + carIndex + laps`, so the
 * lap telemetry and comparison queries (which look up by `lapUid`) can find them.
 */
interface LinkLapTelemetryPort {
  /**
   * Stamp [lapId]/[lapUid] onto every still-unlinked frame for [carIndex] in [sessionUid] whose
   * `laps` counter is below [lapNumber] (i.e. frames captured during this lap or an earlier one
   * that was never linked). Returns the number of frames linked.
   */
  fun linkFramesToLap(
    sessionUid: Uid,
    carIndex: CarId,
