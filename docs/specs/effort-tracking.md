...
- Sessions are created and persisted automatically when `acc-client` broadcasts a new session state (via
  `CreateSessionService`, `StartSessionService`, `UpdateSessionService`, `FinishSessionService`).
- Session metadata stored: track, car, session type, simulator, `startedAt`, `endedAt`. Environment
