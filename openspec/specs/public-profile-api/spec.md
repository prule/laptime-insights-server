...
Generate the public profile snapshot from local Session and Lap data and serve it over HTTP, with a runtime on/off toggle persisted in config and exposed as a HATEOAS action. The frontend consumes the snapshot dynamically via the index resource's `_links`, gated by the `enabledFeatures` flag.
