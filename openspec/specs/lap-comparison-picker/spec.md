# lap-comparison-picker

## Purpose

Defines how a user selects the two laps to compare on the Compare screen: the
shared track axis, latest-session seeding, the anchor (with its my-fastest →
session-best fallback), and the ranked same-track challenger leaderboard with its
scope / driver / car toggles and row tagging. Laps can only be compared within a
single track, so track is the spine the whole selection hangs off.
