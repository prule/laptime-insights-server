...
Defines the cross-cutting query behaviour shared by lap and session search: how sort fields are resolved (unknown fields skipped, not fatal) and how the `from`/`to` date-range bounds are interpreted.
