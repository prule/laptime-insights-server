...
function compareLaps(sort) {
    if (!sort)
        return (a, b) => a.lapNumber - b.lapNumber;
    const [field, order] = sort.split(",")[0].split(":");
    const direction = (order ?? "ASC").toUpperCase() === "DESC" ? -1 : 1;
    return (a, b) => {
        const av = a[field] ?? 0;
...
export async function mockHandler(path) {
    ...
    if (pathname === "/api/1/sessions") {
        ...
        if (uid)
            items = items.filter((s) => s.uid === uid);
        if (car)
            items = items.filter((s) => s.car === car);
        if (track)
            items = items.filter((s) => s.track === track);
        if (simulator)
            items = items.filter((s) => s.simulator === simulator);
        if (from)
        ...
        return delay(paged(items, page, size));
    }
    ...
    if (pathname === "/api/1/laps") {
        ...
        const simulator = query.get("simulator");
        if (uid)
            items = items.filter((l) => l.uid === uid);
        if (sessionUid)
            items = items.filter((l) => l.sessionUid === sessionUid);
        if (personalBest === "true")
            items = items.filter((l) => l.personalBest);
        if (personalBest === "false")
            items = items.filter((l) => !l.personalBest);
        if (validLap === "true")
            items = items.filter((l) => l.valid);
        if (validLap === "false")
            items = items.filter((l) => !l.valid);
        if (car || track || simulator) {
            // Mirror the backend's SESSION join: filter laps by their owning
            // session's car / track / simulator.
            const sessionsByUid = new Map(SESSIONS.map((s) => [s.uid, s]));
            items = items.filter((lap) => {
                const session = sessionsByUid.get(lap.sessionUid);
                if (!session)
                    return false;
                if (car && session.car !== car)
                    return false;
                if (track && session.track !== track)
                    return false;
                if (simulator && session.simulator !== simulator)
                    return false;
                return true;
            });
        }
