...
export function useUrlState(): [
  URLSearchParams,
  (key: string, value: string | number | boolean | undefined | null) => void,
  (updates: Record<string, string | number | boolean | undefined | null>) => void,
] {
...
export function getString(params: URLSearchParams, key: string): string | undefined {
  const v = params.get(key);
  return v && v.length > 0 ? v : undefined;
}

/** Read a 1-based positive integer param with a default. */
export function getInt(params: URLSearchParams, key: string, fallback: number): number {
  const v = params.get(key);
...
export function getBool(params: URLSearchParams, key: string, fallback = false): boolean {
  const v = params.get(key);
